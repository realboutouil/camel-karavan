...
public class KubernetesService {
    ...
    public Map<String, String> getPartOfLabels() {
        ...
        labels.put(LABEL_PART_OF, properties.appName());
        return labels;
    }
...
//        Delete old build pod
    ...
    private Map<String, String> getRuntimeLabels() {
        Map<String, String> labels = new HashMap<>();
        labels.put(properties.openshift().orElse(false) ? "app.openshift.io/runtime" : "app.kubernetes.io/runtime", CAMEL_PREFIX);
        return labels;
    }
    ...
    private Pod getBuilderPod(String name, Map<String, String> labels, String configFragment, boolean hasDockerConfigSecret) {
        ...
        if (properties.privateKeyPath().isPresent()) {
            volumes.add(new VolumeBuilder().withName(PRIVATE_KEY_SECRET_KEY)
                    .withSecret(new SecretVolumeSourceBuilder().withSecretName(properties.secretName()).withItems(
        ...
        PodSpec spec = new PodSpecBuilder()
                ...
                .withServiceAccount(properties.builder().serviceAccount())
                .withVolumes(volumes)
                .build();
    ...
    public void runDevModeContainer(String projectId, Boolean verbose, Boolean compile, Map<String, String> files, String projectDevmodeImage, String deploymentFragment, Map<String, String> labels, Map<String, String> envVars) {
        ...
        try (KubernetesClient client = kubernetesClient()) {
            if (properties.devmode().createM2()) {
    ...
    private Pod getDevModePod(String name, Boolean verbose, Boolean compile, Map<String, String> labels, String projectDevmodeImage, String deploymentFragment, Map<String, String> envVars) {
        ...
        Container container = new ContainerBuilder()
                .withName(name)
                .withImage(projectDevmodeImage != null ? projectDevmodeImage : properties.devmode().image())
                .withPorts(port)
        ...
        podSpec.setServiceAccount(properties.devmode().serviceAccount());
        if (properties.devmode().createM2()) {
    ...
    public Secret getKaravanSecret() {
        try (KubernetesClient client = kubernetesClient()) {
            return client.secrets().inNamespace(getNamespace()).withName(properties.secretName()).get();
        }
    }

    public String getKaravanSecret(String key) {
        try (KubernetesClient client = kubernetesClient()) {
            Secret secret = client.secrets().inNamespace(getNamespace()).withName(properties.secretName()).get();
    ...
    public boolean isOpenshift() {
        return properties.openshift().orElse(false);
    }
